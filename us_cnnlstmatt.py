...
# ---------- 1. 数据加载 (Ultrasound + labels + actions) ----------
# 这部分代码在脚本开始时执行一次
with h5py.File('us_train_val_data_subject7filtered.mat', 'r') as f:
    ...
    train_actions_np = f['all_train_actions'][()]
    val_inputs_np    = f['all_valid_inputs'][()]
    val_labels_np    = f['all_valid_labels'][()]
    val_actions_np   = f['all_valid_actions'][()]

with h5py.File('us_test_data_subject7filtered.mat', 'r') as f:
    test_inputs_np   = f['all_test_inputs'][()]
    test_labels_np   = f['all_test_labels'][()]
    test_actions_np  = f['all_test_actions'][()]
...
train_actions = torch.tensor(train_actions_np.reshape(-1), dtype=torch.long)
val_actions   = torch.tensor(val_actions_np.reshape(-1),   dtype=torch.long)
test_actions  = torch.tensor(test_actions_np.reshape(-1),  dtype=torch.long)

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
train_inputs,  train_actions,  train_labels  = (
    torch.tensor(train_inputs_np, dtype=torch.float32).to(device),
    train_actions.to(device),
    train_labels.to(device)
)
val_inputs,    val_actions,    val_labels    = (
    torch.tensor(val_inputs_np,   dtype=torch.float32).to(device),
    val_actions.to(device),
    val_labels.to(device)
)
test_inputs,   test_actions,   test_labels   = (
    torch.tensor(test_inputs_np,  dtype=torch.float32).to(device),
    test_actions.to(device),
    test_labels.to(device)
)

# ---------- 2. DataLoader 定义 ----------
BATCH = 64
train_loader = DataLoader(TensorDataset(train_inputs, train_actions, train_labels),
                          batch_size=BATCH, shuffle=True)
val_loader   = DataLoader(TensorDataset(val_inputs,   val_actions,   val_labels),
                          batch_size=BATCH, shuffle=False)
test_loader  = DataLoader(TensorDataset(test_inputs,  test_actions,  test_labels),
                          batch_size=BATCH, shuffle=False)

# ---------- 3. 对齐 SensorFusion USBranch 的模型 ----------
# 模型定义保持不变
class CNN_LSTM_SpatialAttn_US(nn.Module):
    def __init__(self, T, C, H, W, num_actions, out_dim,
                 d_emb=64, num_heads=4, drop=0.0, lstm_hidden=64):
        super().__init__()
        self.T = T
    ...
    def forward(self, x, action_id):
        ...
        x_bt = self.conv(x_bt)
        emb = x_bt.view(B*T, self.Cc, self.Hc*self.Wc).permute(0,2,1)
        emb = self.proj(emb)
        a = self.act_emb(action_id)
        a = a.unsqueeze(1).expand(-1, T, -1).reshape(B*T,1,-1)
...
def train_single_run(run_index):
    ...
    model_filename = f'models/us_cnnlstmatt_subject7{run_index}.pth'
    ...
    plt.legend()
    plt.tight_layout()
    figure_filename = f'figures/us_aligned_loss_no11_cls_run{run_index}.png'
