...
# US labels, actions (must match EMG labels for regression)
...
# ---------- 5. 融合主干 & 头部 (Ablated: LSTM removed) ----------
class SensorFusion(nn.Module):
    def __init__(self, T, us_C, us_H, us_W, emg_C, emg_H, emg_W,
        ...
        self.fc_reg = nn.Linear(fuse_dim, out_dim)
        self.fc_cls = nn.Linear(fuse_dim, num_actions)

    def forward(self, us, emg, act):
        seq_us = self.us_branch(us, act)
        seq_em = self.emg_branch(emg, act)
        seq = torch.cat([seq_us, seq_em], dim=2)
        aggregated_feat = torch.mean(seq, dim=1)
        return self.fc_reg(aggregated_feat), self.fc_cls(aggregated_feat)

### --- 新的训练函数 --- ###
def train_single_run(run_index):
    """
    为给定的运行索引（run_index）执行一次完整的训练和保存。
    """
    print(f"\n===== [ 开始执行第 {run_index} 次训练 ] =====")
    ...
    for epoch in range(1, 501):
        ...
        for us_b, emg_b, a_b, y_b in train_loader:
            opt.zero_grad()
            pr, pc = model(us_b, emg_b, a_b)
            loss_r = crit_r(pr, y_b)
            loss_c = crit_c(pc, a_b)
            (loss_r + alpha * loss_c).backward()
    ...
    # ---------- 8. 保存 & 绘图 ----------
    if best_wts is None:
        print(f"  警告: 第 {run_index} 次训练未找到最佳权重，不保存任何内容。")
        return

    print(f"\n  第 {run_index} 次训练完成，正在保存最佳模型和图片...")
    os.makedirs('models', exist_ok=True)
    os.makedirs('figures', exist_ok=True)
    
    # --- 已更改 --- 动态的文件名
    model_filename = f'models/sensorfusion_cnnatt_subject7{run_index}.pth'
    ...
    figure_filename = f'figures/sensorfusion_ablation_no_lstm_loss_run{run_index}.png'
...
if __name__ == '__main__':
    ...
    for i in range(NUM_RUNS):
        ...
        train_single_run(run_index=run_idx)
